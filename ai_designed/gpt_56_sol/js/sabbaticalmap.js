...
// Marker icons
function createMarkerIcon(className, visualSize) {
    return L.divIcon({
        className: 'map-marker-hitbox ' + className,
        html: '<span class="marker-dot ' + className + '" style="width:' + visualSize + 'px;height:' + visualSize + 'px"></span>',
...
function addParentMarkers() {
    ...
    if (latLngs.length > 1) {
        var polyline = L.polyline(latLngs, { color: '#9e281b', opacity: 0.5, weight: 2, dashArray: '2, 5' }).addTo(map);
...
function addSubMarkers(locations) {
    ...
    locations.forEach(function(location, index) {
        ...
        var opts = { color: '#9e281b', opacity: 0, weight: 2 };
