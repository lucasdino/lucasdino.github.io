...
// Expose lightbox globally so the physics polaroids can use it
...
// Intercept map click to spawn photo in physics world!
document.getElementById('travel_map').addEventListener('click', function(e) {
    if (e.target.classList.contains('lightbox-trigger') || e.target.classList.contains('popup-image')) {
        e.stopPropagation();
        
        // Spawn in gravity physics sandbox!
        var imageSrc = e.target.getAttribute('src');
        var cityName = e.target.alt || "Sabbatical Memory";
        
        if (window.spawnPhysicalPhotoFromMap) {
            window.spawnPhysicalPhotoFromMap(imageSrc, cityName);
        } else {
            // Fallback to standard lightbox if physics is missing
        ...
        if (details.drawMarker) {
            ...
            marker._popupHtml = `
                <div class="popup-content">
                    ...
                    ${details.parentPhoto ? `<div class="popup-image-container"><img src="${details.parentPhoto}" class="popup-image lightbox-trigger" alt="${category}"></div>` : ''}
                </div>
            `;
            ...
            if (details.parent) {
                ...
                // If it doesn't have sublocations but has a photo, clicking spawns it
                if (details.parentPhoto) {
                    marker.on('click', function() {
                        if (window.spawnPhysicalPhotoFromMap) {
                            window.spawnPhysicalPhotoFromMap(details.parentPhoto, category);
                        }
                    });
                }
        ...
        setTimeout(function() {
            ...
            subMarkerObj._popupHtml = `
                <div class="popup-content">
                    ...
                    ${location.photo ? `<div class="popup-image-container"><img src="${location.photo}" class="popup-image lightbox-trigger" alt="${location.name}"></div>` : ''}
                </div>
            `;
            subMarkerObj._hasImage = !!location.photo;
            
            // If sublocation has photo, click marker to drop it
            if (location.photo) {
                subMarkerObj.on('click', function() {
                    if (window.spawnPhysicalPhotoFromMap) {
                        window.spawnPhysicalPhotoFromMap(location.photo, location.name);
                    }
                });
            }
